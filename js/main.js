...
init();}
function bannerSlide(){var box=$('.ag7-bannerslide .slidebox'),buttons=$('.ag7-bannerslide .slidelist'),mask=$('.ag7-bannerslide .mask'),h1=$('.ag7-bannerslide h2'),h2=$('.ag7-bannerslide h3'),list=box.children('li'),len=list.length,curIndex=0,nextIndex=1,speed=800,timer=null,delay=null,duration=4000;var setTitle=function(index){var li=buttons.find('li').eq(index);h1.html(li.data('h1'));h2.html(li.data('h2'));}
...
mask.stop(true,true).animate({top:buttons.find('li').eq(next).position().top+10},300,'easeOutExpo');setTitle(next);curIndex=next;}
